...
f = open('AOTC2018_1.txt', 'r')

freq_change = [x.strip() for x in f]

final = 0
for x in freq_change:
    final += int(x)
...
freq_list = [0]
final = 0
freq_change_cycle = itertools.cycle(freq_change)

for x in freq_change_cycle:
    final += int(x)
    if final in freq_list:
        print(final)
        break
    else:
        freq_list.append(final)
